...
import 'package:holom_said/features/personalzation/providers/profile_providers.dart';
...
class UserProfileNotifier extends BaseProfileNotifier {
  ...
  Future<BaseProfile?> getCurrentProfile() async {
    try {
      ...
      if (userId == null) {
        throw AuthException(S.current.authError);
      }
  ...
  Future<String?> pickAndUploadImage(String userId) async {
    state = await AsyncValue.guard(() async {
      try {
        ...
        final context = ErrorUtils.navigatorKey.currentContext;
        if (context == null) {
          throw Exception('Context not available for permission request');
        }
        final imageUrl = await Helpers.uploadImage(context);
